...
class PacketLoss extends Component {
  ...
  constructor(props) {
    ...
    console.log("PCKET LOAA", props)
    
    var cloneedObj = Object.assign({}, props.config); 
    ...
    let loss = [0, 0, 0];
      ...
      const packetLoss = loss;



        console.log("Cloned object", cloneedObj)
    for (let [packetLossKey, packetLossValue] of Object.entries(cloneedObj)) {
      console.log("*******************************", packetLossKey)
      if(packetLossKey == "rotate") {   
        props.config[packetLossKey] = false; 
             }
